...
+ (SecKeyRef) GeneratePrivateKey:(NSString *)privatekeyStr{
    if(!privatekeyStr){
        return nil;
    }
    SecKeyRef keyRef =NULL;
    
    keyRef = [RSA addPrivateKey:privatekeyStr];
    if(!keyRef){
        return nil;
    }
    return keyRef;
...
+ (SecKeyRef)GenerateSecretKey:(NSData *)key{
    if(!key){
        return nil;
    }
    SecKeyRef keyRef =NULL;
    //a tag to read/write keychain storage
    NSString *tag = @"RSAUtil_SecretKey";
    NSData *d_tag = [NSData dataWithBytes:[tag UTF8String] length:[tag length]];
    
    // Delete any old lingering key with the same tag
    NSMutableDictionary *privateKey = [[NSMutableDictionary alloc] init];
    [privateKey setObject:(__bridge id) kSecClassKey forKey:(__bridge id)kSecAttrKeyClassSymmetric];
    [privateKey setObject:(__bridge id) kSecAttrKeyType forKey:(__bridge id)kSecAttrKeyType];
    [privateKey setObject:d_tag forKey:(__bridge id)kSecAttrApplicationTag];
    SecItemDelete((__bridge CFDictionaryRef)privateKey);
    
    // Add persistent version of the key to system keychain
    [privateKey setObject:key forKey:(__bridge id)kSecValueData];
    [privateKey setObject:(__bridge id) kSecAttrKeyClassSymmetric forKey:(__bridge id)
     kSecAttrKeyClass];
...
+ (NSData *)decryptSecretKeyString:(NSString *)str privateKey:(NSString *)privKey{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [RSA decryptData:data privateKey:privKey];
    NSString *ret =[[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
...
+ (NSString *)decryptString:(NSString *)str publicKey:(NSString *)pubKey{
    ...
    NSString *decodedStringpk = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    //NSString *decodedStringpk = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
	//NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
	return decodedStringpk;
}
